...
function bookingForm() {
  return `<form data-booking-form class="scheduler-grid two"><label class="scheduler-field"><span class="scheduler-label-text">Full name <span class="scheduler-required">*</span></span><input class="scheduler-input" name="candidateName" required></label><label class="scheduler-field"><span class="scheduler-label-text">Mobile number <span class="scheduler-required">*</span></span><input class="scheduler-input" name="phone" inputmode="numeric" pattern="\\d{10}" maxlength="10" required placeholder="10 digit mobile number"></label><label class="scheduler-field">Resume link<input class="scheduler-input" type="url" name="resumeLink" placeholder="https://..."></label><label class="scheduler-field">LinkedIn / GitHub / portfolio<input class="scheduler-input" type="url" name="profileLink" placeholder="https://..."></label><label class="scheduler-field" style="grid-column:1/-1">Optional note<textarea class="scheduler-textarea" name="candidateNote"></textarea></label><div class="scheduler-panel" style="grid-column:1/-1"><h2 class="anthra-section-title text-2xl">Available Slots <span class="scheduler-required">*</span></h2><div class="scheduler-grid two mt-4">${availableSlots.map((slot) => `<label class="scheduler-slot"><input type="radio" name="slotId" value="${slot.id}" required><strong>${formatSlotRange(slot.startAt, slot.endAt)}</strong></label>`).join("")}</div></div><div><button class="scheduler-button primary" type="submit">Book Interview Slot</button></div></form>`;
}
